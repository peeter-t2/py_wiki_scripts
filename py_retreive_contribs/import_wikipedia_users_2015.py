...
full_array= array1+array2+array3+array4+array5
#print(full_array)

#Find the right links to articles and change them to the discussion pages.
test1 = [wikipedia.page(link).links for link in array5] #Removed the numerator from the tests.
...
test4 = [wikipedia.page(link).links for link in matching4]
merged4 = list(itertools.chain.from_iterable(test4))
...
#lllinks=[]
#for name in matching1:
#    m = wikipedia.page(name)
#    link=m.links
#    alllinks.append(link)

cPickle.dump(merged1, open('articles2015.p', 'wb'))

alllinks=[]
for name in matching4:
    m = wikipedia.page(name)
    link=m.links
    alllinks.append(link)
cPickle.dump(merged4, open('articles2014.p', 'wb'))
